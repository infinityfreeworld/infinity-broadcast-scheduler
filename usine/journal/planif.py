...
# Fondateur, 16/09/2026 : Iggy devient un CAMÉLÉON — même allure, mais il gobe une mouche une à deux fois par émission et il
...
MOUCHE = (" A small fly buzzes into the frame; his long tongue shoots out in a flash and catches it, he swallows and calmly goes on "
          "talking.")
MOUCHES_PAR_JT = 2         # « au moins une à 2 fois par présentation » — jamais deux fois de suite, jamais dans une suite de réplique
...
def main(jt_chemin, dossier):
    ...
    colores = set(lancements[1::2])                                    # un sujet sur deux à partir du 3e : il change de teinte
    mouches = set(de.sample(lancements, min(MOUCHES_PAR_JT, len(lancements))))
    iggy_couleur = None
    if colores:   # une teinte ne vaut son image (~45 s de carte) que s'il y a des lancements à colorer
        images.append({"cle": "iggy-couleur", "sources": ["entrees/persos/iggy.png"], "graine": 200 + de.randrange(50),
                       "consigne": IGGY_COULEUR.format(couleur=de.choice(COULEURS))})
        iggy_couleur = "resultats/images/iggy-couleur.png"
    ...
    for k, s in enumerate(sujets, 1):
        ...
        teinte = iggy_couleur if (k in colores and iggy_couleur) else None
